export const TILE_DUKE = 'duke';
export const TILE_FOOTMAN = 'footman';
export const TILE_KNIGHT = 'knight';
export const TILE_PRIEST = 'priest';
export const TILE_SEER = 'seer';
export const TILE_PIKEMAN = 'pikeman';
export const TILE_RANGER = 'ranger';
export const TILE_WIZARD = 'wizard';
export const TILE_GENERAL = 'general';
export const TILE_LONGBOWMAN = 'longbowman';
export const TILE_ASSASSIN = 'assassin';
// not yet implemented
export const TILE_CHAMPION = 'champion';
export const TILE_MARSHALL = 'marshall';
export const TILE_BOWMAN = 'bowman';
export const TILE_DRAGOON = 'dragoon';
// Enhanced tiles
export const TILE_DUCHESS = 'duchess';
export const TILE_ORACLE = 'oracle';
export const HIGHLIGHTS_DUKES_FOOTMEN = 'highlights-dukes-footmen';

export const SLIDE_LEFT = 'SLIDE_LEFT';
export const SLIDE_RIGHT = 'SLIDE_RIGHT';
export const SLIDE_UP = 'SLIDE_UP';
export const SLIDE_DOWN = 'SLIDE_DOWN';
export const SLIDE_DIAG_UP_LEFT = 'SLIDE_DIAG_UP_LEFT';
export const SLIDE_DIAG_UP_RIGHT = 'SLIDE_DIAG_UP_RIGHT';
export const SLIDE_DIAG_DOWN_LEFT = 'SLIDE_DIAG_DOWN_LEFT';
export const SLIDE_DIAG_DOWN_RIGHT = 'SLIDE_DIAG_DOWN_RIGHT';
export const JUMPSLIDE_UP = 'JUMPSLIDE_UP';
export const JUMPSLIDE_DOWN = 'JUMPSLIDE_DOWN';
export const JUMPSLIDE_LEFT = 'JUMPSLIDE_LEFT';
export const JUMPSLIDE_RIGHT = 'JUMPSLIDE_RIGHT';
export const JUMPSLIDE_DIAG_UP_LEFT = 'JUMPSLIDE_DIAG_UP_LEFT';
export const JUMPSLIDE_DIAG_UP_RIGHT = 'JUMPSLIDE_DIAG_UP_RIGHT';
export const JUMPSLIDE_DIAG_DOWN_LEFT = 'JUMPSLIDE_DIAG_DOWN_LEFT';
export const JUMPSLIDE_DIAG_DOWN_RIGHT = 'JUMPSLIDE_DIAG_DOWN_RIGHT';

// these are used in descriptions of moves for tiles
export const RULETYPE_MOVE = 'move';
export const RULETYPE_JUMP = 'jump';
export const RULETYPE_SLIDE = 'slide';
export const RULETYPE_JUMPSLIDE = 'jumpslide';
export const RULETYPE_STRIKE = 'strike';
export const RULETYPE_COMMAND = 'command';
export const DENYMOVESPACES = 'denyMoveSpaces';

// special ruletypes not for use in moves for tiles
export const RULETYPE_COMMAND_MOVES = 'ruletype_command_moves';